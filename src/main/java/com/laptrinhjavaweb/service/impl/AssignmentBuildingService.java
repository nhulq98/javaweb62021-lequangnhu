...
public class AssignmentBuildingService implements IAssignmentBuildingService {
    ...
    public List<StaffBuildingResponse> getStaffsAssignment(Long buildingId) {
        List<StaffBuildingResponse> staffsAll = findAllStaff();
        List<AssignmentBuildingEntity> staffsOfBuilding = assignmentBuildingRepository.findAllByBuildingId(buildingId);

        List<StaffBuildingResponse> result = new ArrayList<>();

        //convert to staffResponse
        for (int i = 0; i < staffsAll.size(); i++) {
            result.add(staffsAll.get(i));
            for (AssignmentBuildingEntity staff : staffsOfBuilding) {
                if (staffsAll.get(i).getId() == staff.getUser().getId()) {
    ...
    public void updateAssignment(StaffBuildingRequest request) {
        ...
        List<AssignmentBuildingEntity> staffsOfBuildingFromDB = assignmentBuildingRepository.findAllByBuildingId(request.getBuildingId());
        ...
        for (int i = 0; i < staffsOfBuildingFromDB.size(); i++) {
            for (int j = 0; j < staffsFromRequest.size(); j++) {
                if (staffsOfBuildingFromDB.get(i).getUser().getId().equals(staffsFromRequest.get(j).getUser().getId())) {
